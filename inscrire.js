...
function checkMotdepasse(){
    ...
    const validMpdp = /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$ %^&*-]).{8,}$/.test(checkMdp);
    return validMpdp;
}
...
document.getElementById('valider').addEventListener('click', function(event) {
    ...
    if (isUsernameValid && isEmailValid && isPasswordValid && isPasswordConfirmed) {
       ...
       const utilisateurs = JSON.parse(localStorage.getItem('utilisateurs')) || [];
        ...
        const chercheUtilisateur = utilisateurs.find(check => check.username === utilisateur.value);

        if(chercheUtilisateur){
            document.getElementById('confirmation').style.color ='red'
            document.getElementById('confirmation').innerText = "Le Nom d'utilisateur existe déja. Connecter vous ou changez de nom"
        }
        else{
            ...
            document.getElementById('confirmation').innerText = "Votre inscription est valider. Vous pouvez vous Connecter"
        ...
        if (!isPasswordConfirmed) alert("La confirmation du mot de passe ne correspond pas.");
    }
});
...
document.getElementById('mdp').addEventListener('input', function(){
    ...
    strengthBar.innerHTML = '<div style="width: ' + strength + '%; background-color: ' + getStrengthColor(strength) + ';"></div>';
});
